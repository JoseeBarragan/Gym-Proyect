...
export function UserFormModal({ isOpen, onClose, onSubmit, user }: UserFormModalProps) {
  ...
  useEffect(() => {
    if (user) {
      ...
      reset({ email: '', contrasena: '', nombre: '', apellido: '', telefono: '', tipoUsuario: 'Socio' });
    }
  }, [user, reset]);
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
      <div className="admin-card" style={{ width: '100%', maxWidth: '500px', backgroundColor: '#1a1d24' }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {!user && (
             <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
               <label style={{ fontSize: '0.875rem', color: '#94a3b8' }}>Rol</label>
               <select {...register('tipoUsuario', { required: true })} style={{ padding: '0.5rem', borderRadius: '0.375rem', backgroundColor: '#0f1115', border: '1px solid #2d323e', color: '#fff' }}>
